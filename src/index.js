...
export default class Veins {
    ...
    _open() {
        ...
        this.auth().then((auth) => {
            ...
            if (this.socket === false) {
                if (this.error) {
                    _this.error('Websockets not supported');
                }
                return;
            }
    ...
    _sockMessage(sock, ev) {
        ...
        function handleMsg(msg) {
            ...
            if (msg.error) {
                if (_this.error) {
                    _this.error(`Websocket failed: ${msg.error.msg} (${msg.error.code})`);
                }
                return;
            }
            // If we have the key
        ...
        // Else we got a blob
            ...
            r.addEventListener('loadend', () => {
                // Parse the data
                const oMsg = JSON.parse(r.result);
                // Handle it
                handleMsg(oMsg);
            });
                ...
                if (callback === this.keys[key][i]) {
                    ...
                    if (this.keys[key].length === 0) {
                        ...
                        if (empty(this.keys)) {
                            ...
                            this.close = true;
                            if (this.socket) {
                                this.socket.close(1000, 'nothing else to track');
                            }
                        }
                    }
